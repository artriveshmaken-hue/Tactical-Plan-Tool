/* ═══════════════════════════════════════════════════════════
   APP.JS — Upload, state, navigation, KPI strip
═══════════════════════════════════════════════════════════ */
...
function setupUpload() {
  ...
  $('btn-analyze').addEventListener('click', async () => {
    ...
    try {
      ...
      $('dash-sub').textContent =
        `${baseFile.name} (2026) vs ${reviewFile.name} (2027) — ${review.activities.length} activities`;
  ...
  $('btn-new-upload')?.addEventListener('click', () => {
    ...
    APP.baseline=APP.review=APP.comparison=null; APP.violations=[];
    destroyCharts();
    $('input-baseline').value=$('input-review').value='';
...
// ── KPI Strip ─────────────────────────────────────────────
function renderKPIs() {
  const acts27 = APP.review.activities||[];
  const acts26 = APP.baseline.activities||[];
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  console.log('DCT Tactical Plan Review Tool — Ready');
});
