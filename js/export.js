...
function exportViolationsToExcel(violations){
  const rows=violations.map((v,i)=>({'#':i+1,'Tactical ID':v.activityId,'Rule ID':v.ruleId,'Category':v.category,'Rule Name':v.ruleName,'Severity':v.severity,'Tier':`Tier ${v.tier}`,'Region':v.region,'Market':v.market,'Activity Type':v.activityType,'Activity / Item':v.activityName,'Detail':v.detail,'Status':v.status==='accepted'?'Accepted':v.status==='action-required'?'Action Required':'Pending Review','What Needs to Change':v.comment||''}));
  const ws=XLSX.utils.json_to_sheet(rows);
  ws['!cols']=[{wch:4},{wch:12},{wch:8},{wch:14},{wch:42},{wch:10},{wch:7},{wch:14},{wch:22},{wch:22},{wch:36},{wch:70},{wch:16},{wch:50}];
...
function exportViolationsToCSV(violations){
  const headers=['Tactical ID','Rule ID','Category','Rule Name','Severity','Tier','Region','Market','Activity Type','Activity','Detail','Status','What Needs to Change'];
  const rows=violations.map(v=>[v.activityId,v.ruleId,v.category,v.ruleName,v.severity,`Tier ${v.tier}`,v.region,v.market,v.activityType,v.activityName,v.detail,v.status==='accepted'?'Accepted':v.status==='action-required'?'Action Required':'Pending',v.comment||'']);
